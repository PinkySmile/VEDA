...
void	mainMenu()
{
	...
	//var -= 0.0001 * var;
	//sfMusic_setPitch(((sfMusic **)game.ressources.musics.content)[MAIN_MENU_MUSIC], var);
	if (!sfRenderWindow_isOpen(game.ressources.window))
		return;
	if (((sfMusic **)game.ressources.musics.content)[MAIN_MENU_MUSIC] && sfMusic_getStatus(((sfMusic **)game.ressources.musics.content)[MAIN_MENU_MUSIC]) != sfPlaying)
		sfMusic_play(((sfMusic **)game.ressources.musics.content)[MAIN_MENU_MUSIC]);
