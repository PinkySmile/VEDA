...
void	audio(game_t *game)
{
	...
	text("Sound effects", game, 5, 8, false);
	text("Musics", game, 5, 56, false);
	nbrs[0] = concatf("%i%%", game->settings.sfxVolume);
	nbrs[1] = concatf("%i%%", game->settings.musicVolume);
	if (nbrs[0])
		text(nbrs[0], game, 455, 8, false);
	if (nbrs[1])
		text(nbrs[1], game, 455, 56, false);
	free(nbrs[0]);
	free(nbrs[1]);
}
...
void	controls(game_t *game)
{
	...
	for (int i = 0; game->languages[getLanguage(game->languages, game->settings.lang_id)].keys[i] && i < NB_OF_KEYS; i++) {
		...
		text(game->languages[getLanguage(game->languages, game->settings.lang_id)].keys[i], game, 5 + i / 10 * 272, i % 10 * 48 + 15, false);
	}
}
